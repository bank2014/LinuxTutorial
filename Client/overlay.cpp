...
void Overlay::RenderMenu()
{
	...
	if (ImGui::BeginTabBar(XorStr("tap")))
	{
		if (ImGui::BeginTabItem(XorStr("Main")))
		{
			ImGui::Checkbox(XorStr("ES"), &esp);
			ImGui::Checkbox(XorStr("ea-im"), &aim_enable);
			if (aim_enable)
			{
				ImGui::SameLine();
				ImGui::Checkbox(XorStr("visib1e check?"), &vis_check);
				ImGui::SameLine();
				ImGui::Checkbox(XorStr("no-BanDong/swae2"), &aim_no_recoil);
				if (vis_check)
				{
					aim = 2; // aim only when visible
				}
				else
				{
					aim = 1; // just aim
			...
			{
				aim = 0; // aimbot disabled
			}

			ImGui::Checkbox(XorStr("illuminati i"), &item_glow);
			ImGui::Checkbox(XorStr("illuminati pl"), &player_glow);
			ImGui::EndTabItem();
		}

		if (ImGui::BeginTabItem(XorStr("Config")))
		{
			ImGui::SliderFloat(XorStr("Max distance"), &max_dist, 100.0f * 40, 800.0f * 40, "%.2f");
			ImGui::SameLine();
			ImGui::Text("(%d meters)", (int)(max_dist / 40));

			ImGui::SliderFloat(XorStr("smo@@oothing"), &smooth, 12.0f, 150.0f, "%.2f");

			ImGui::SliderFloat(XorStr("maximum fov"), &max_fov, 5.0f, 250.0f, "%.2f");

			ImGui::SliderInt(XorStr("aim at the (bon id):"), &bone, 0, 3);
			ImGui::EndTabItem();
		}

		if (ImGui::BeginTabItem(XorStr("Visua1s")))
		{
			ImGui::Checkbox(XorStr("bock su"), &v.box);;
			ImGui::Checkbox(XorStr("name"), &v.name);
			ImGui::Checkbox(XorStr("line"), &v.line);
			ImGui::Checkbox(XorStr("dist"), &v.distance);
			ImGui::Checkbox(XorStr("che ryuk bax"), &v.healthbar);
			ImGui::Checkbox(XorStr("shil dew bax"), &v.shieldbar);
			ImGui::EndTabItem();
		}
...
DWORD Overlay::CreateOverlay()
{
	...
	while (running)
	{
		...
		if (GetWindowLong(overlayHWND, GWL_STYLE) != nv_edit) SetWindowLong(overlayHWND, GWL_STYLE, nv_edit);
		if (show_menu)
		{
			ClickThrough(false);
		}
		...
		if (IsKeyDown(VK_XBUTTON2) && !k_ins && ready)
		{
			show_menu = !show_menu;
			ClickThrough(!show_menu);
			k_ins = true;
		}
		else if (!IsKeyDown(VK_XBUTTON2) && k_ins)
		{
			k_ins = false;
		}

		if (show_menu) RenderMenu();
		else RenderInfo();
