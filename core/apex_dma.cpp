...
const char* cl_proc = "Logitech G HUB.exe";
const char* ap_proc = "R5Apex.exe";
uint64_t add_off = 0x4c870; //Client "add" offset
...
static void set_vars(uint64_t add_addr)
{
	...
	client_mem.Read<uint64_t>(add_addr + sizeof(uint64_t)*15, spectators_addr);
	uint64_t allied_spectators_addr = 0;
	client_mem.Read<uint64_t>(add_addr + sizeof(uint64_t)*16, allied_spectators_addr);
	uint64_t shooting_addr = 0;
	client_mem.Read<uint64_t>(add_addr + sizeof(uint64_t)*17, shooting_addr);
